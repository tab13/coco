...
export class CreditCardInput extends Component<Props, State> {
  ...
  handleCardNumberBlur = (
    ...
    if (!payment.fns.validateCardNumber(value)) {
      ...
      this.setState({errorMessageCCNumber: this.translate(message), showErrorMessageCCNumber: true});
    }
    ...
    if (
      ...
      this.setState({
          errorMessageCCNumber: this.translate('This type card is not supported'),
          showErrorMessageCCNumber: true
      });
    }
    ...
    this.setState({showErrorMessageCCNumber: false});
    ...
    if (cardTypeLengths) {
      ...
      for (let length of cardTypeLengths) {
        ...
        if (this.props.autoFocus && cardNumberLength === lastCardTypeLength) {
          ...
          this.setState({
              errorMessageCCNumber: this.translate('Please enter a valid card number'),
              showErrorMessageCCNumber: true
          });
        }
      }
  ...
  handleCardExpiryBlur = (
    ...
    if (expiryError) {
      ...
      this.setState({
          errorMessageCardExpiry: this.translate(expiryError),
          showErrorMessageCardExpiry: true
      });
      this.setFieldInvalid(expiryError, { state: 'ccExpDateErrorText' });
    } else {
        this.setState({
            showErrorMessageCardExpiry: false
        });
    }
  ...
  handleCardCVCBlur = (
    ...
    if (!cvcIsValid) {
      ...
      this.setState({
          errorMessageCardCVC: this.translate(message),
          showErrorMessageCardCVC: true
      });
      this.setFieldInvalid(message, {
  ...
  handleCardCVCChange = (
    ...
    if (!payment.fns.validateCardCVC(CVC, cardType) && this.props.autoFocus) {
      ...
      this.setState({
          errorMessageCardCVC: this.translate('Please enter a valid CSC'),
          showErrorMessageCardCVC: true
      });
    }
  ...
  handleCardZipBlur = (
    ...
    if (!isValidZip(e.target.value)) {
      ...
      this.setState({
          errorMessageCardZip: this.translate('Zip code is invalid'),
          showErrorMessageCardZip: true
      });
    } else {
      this.setState({
          showErrorMessageCardZip: false
      });
    }
